...
# =========================================================
st.set_page_config(page_title="Generador Pro Santiago y Santiago", layout="wide")
...
GIDS = {
    ...
    "PAIS_ORIGEN": "TU_GID_AQUI" # Cambia esto por el GID real de tu hoja de países
}
...
with st.container():
    ...
    lote = st.text_input("🔢 Número de Lote", help="Escribe el lote para activar la descarga")
    ...
    with fe3:
        default_cad = date.today() + timedelta(days=7)
        f_cad_val = st.date_input("Fecha de Caducidad", value=default_cad)

# =========================================================
# 3. LÓGICA DEL PDF (10x16 CM - ANTI SALTO DE PÁGINA)
# =========================================================
def generar_pdf(d):
    # Formato 100x160mm, sin salto de página automático
    pdf = FPDF(orientation='P', unit='mm', format=(100, 160))
    pdf.set_auto_page_break(auto=False)
    pdf.add_page()
    pdf.rect(5, 5, 90, 150) # Marco
    ...
    pdf.set_xy(8, y_dyn)
    pdf.set_font("Arial", 'B', 9); pdf.cell(20, 5, "CONTIENE: ")
    pdf.set_font("Arial", '', 9); pdf.cell(0, 5, d['alergenos'], ln=True)
    ...
    for lab, val in [("ZONA DE CAPTURA:", d['zona']), ("ARTE DE PESCA:", d['arte']), 
    ...
    # --- BLOQUES ANCLADOS AL FINAL (Evita hoja 2) ---
    # Conservación
    pdf.line(8, 92, 92, 92)
    pdf.set_xy(8, 94); pdf.set_font("Arial", 'B', 9)
...
if nombre_base != "Selecciona una opción" and lote:
    ...
    st.write("---")
    st.download_button(
        label="📥 DESCARGAR ETIQUETA PDF (10x16)",
        data=pdf_data,
        file_name=f"Etiqueta_{lote}.pdf",
        mime="application/pdf",
        use_container_width=True
    )
    st.success("✅ PDF generado correctamente sin saltos de página.")
else:
    st.info("👋 Selecciona un producto y escribe el Lote para descargar.")
